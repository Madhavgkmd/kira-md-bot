...
// ==========================================
// NEW FEATURE: ACCEPT ALL PENDING REQUESTS
// ==========================================
{
    name: "acceptall",
    ...
    async execute(sock, msg) {
        ...
        try {
            ...
            if (!response || response.length === 0) {
                ...
                return await sock.sendMessage(jid, { 
                    text: "⚠️ *No pending join requests found in this group!*" 
                }, { quoted: msg });
            }

            // Extract all JIDs of pending users
            const jidsToApprove = response.map(req => req.jid);

            // Approve all in one go
            await sock.groupRequestParticipantsUpdate(jid, jidsToApprove, "approve");

            await sock.sendMessage(jid, { react: { text: "✅", key: msg.key } });
            await sock.sendMessage(jid, { 
                text: `✅ *Successfully approved ${jidsToApprove.length} pending request(s)!*` 
            }, { quoted: msg });
            ...
            await sock.sendMessage(jid, { react: { text: "❌", key: msg.key } });
            await sock.sendMessage(jid, { 
                text: "❌ *Failed to accept requests. Make sure the bot is an Admin in this group!*" 
            }, { quoted: msg });
        }
    }
}
