...
const {
    ...
    DisconnectReason,
    fetchLatestWaWebVersion, // 🔄 Updated to match index.js
    makeCacheableSignalKeyStore,
    Browsers // 🔥 THE FIX: Imported Browsers
...
async function startSubBot(phoneNumber, mainSock = null, requestJid = null, requestMsg = null) {
    ...
    try {
        ...
        subSock = makeWASocket({
            ...
            browser: Browsers.macOS("Chrome"), // 🔥 THE FIX: Official Browser String prevents disconnects!
        ...
        subSock.ev.on("connection.update", async update => {
            try {
                ...
                // QR വന്നാൽ മാത്രം കോഡ് ചോദിക്കുന്നു
        ...
        subSock.ev.on("messages.upsert", async ({ messages, type }) => {
            try {
                if (type === 'append') return; // Anti-Lag Fix
...
async function handleSubMessage(sock, msg, cleanNumber, type) {
    try {
        ...
        // ─────────────────────────────────────
        // PRIVATE MODE (Global & DB Check)
        // ─────────────────────────────────────
        const currentMode = config.botMode || global.botMode || "public";
