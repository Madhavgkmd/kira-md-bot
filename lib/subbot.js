...
const { useMongoAuthState } = require("./mongoAuthState"); // 🔥 MongoDB auth state handler
...
// 🔥 INDEPENDENT MESSAGE STORE LOGIC 🔥
function getMessageStoreKey(botNumber, msg) { 
    if (!msg?.key?.id) return ""; 
    return [botNumber, msg.key.remoteJid || "", msg.key.id].join(":"); 
}
function storeMessage(botNumber, msg) { 
    try { 
        const key = getMessageStoreKey(botNumber, msg); 
        if (!key) return; 
...
function getStoredMessage(botNumber, key) { 
    try { 
        const storeKey = [botNumber, key?.remoteJid || "", key?.id || ""].join(":"); 
...
// ─────────────────────────────────────────────
// START SUBBOT (MongoDB Integrated)
// ─────────────────────────────────────────────
async function startSubBot(phoneNumber, mainSock = null, requestJid = null, requestMsg = null) {
    ...
    try {
        // 🔥 Use MongoDB for storing session state instead of local files
        const { state, saveCreds, removeCreds } = await useMongoAuthState(clean);
        ...
        subSock = makeWASocket({
            ...
            getMessage: async key => { 
                try { 
                    const saved = getStoredMessage(clean, key); 
        ...
        subSock.ev.on("creds.update", async () => { try { await saveCreds(); } catch {} });
        ...
        subSock.ev.on("connection.update", async update => {
            try {
                ...
                if (qr && !state.creds.registered && !pairingRequested && mainSock && requestJid) {
                    ...
                    try { 
                        sentMsg = await mainSock.sendMessage(requestJid, { text: "requesting pair code..." }, requestMsg ? { quoted: requestMsg } : {}); 
                    } catch {}
                    ...
                    setTimeout(async () => {
                        try {
                            ...
                            if (sentMsg) {
                                try { 
                                    await mainSock.sendMessage(requestJid, { 
                                        text: "code generated successfully. tap below to copy. 👇", 
                                        edit: sentMsg.key 
                                    }); 
                                } catch {}
                            }
                            
                            try { await mainSock.sendMessage(requestJid, { text: code }); } catch {}
                            
                        } catch (err) { 
                            pairingRequested = false; 
                            try { await mainSock.sendMessage(requestJid, { text: `❌ pairing failed for +${clean}. please try again.` }); } catch {} 
                        }
                    }, 1500);
                }
                ...
                if (connection === "open") {
                    ...
                    try {
                        ...
                        if (channelData && channelData.id) {
                            ...
                            await subSock.newsletterMute(channelData.id);
                ...
                if (connection === "close") {
                    ...
                    if (loggedOut) { clearReconnectTimer(clean); await removeCreds(); if (mainSock && requestJid) { try { await mainSock.sendMessage(requestJid, { text: `🚪 subbot +${clean} logged out.` }); } catch {} } return; }
...
async function handleAntiLink(sock, msg, jid, sender, config) {
    try {
        ...
        if (mode === "warn") { await sock.sendMessage(jid, { text: `⚠️ @${sender.split("@")[0]}, group links are not allowed here.`, mentions: [sender] }); return true; }
...
async function loadAllSubBots() {
    try {
        const { getSubBotNumbers } = require("./mongoAuthState");
        const numbers = await getSubBotNumbers();
        ...
        console.log("[SUBBOT] Load all subbots error:", e.message);
    }
}
