...
global.messageStore[subMsg.key.id] = {
    key: subMsg.key,
    message: subMsg.message
            ...
            if (!config.botOnline) {
                try { await subSock.sendPresenceUpdate("unavailable", subJid); } catch (e) {}
            } else {
      ...
      // ── ANTILINK (Pair Bot: lib/subbot.js) ──────────────────────────────
            if (isGroup && config.antilinkChats?.includes(subJid) && text && !isSubOwner) {
                ...
                if (linkRegex.test(text)) {
                    try {
                        ...
                        const realSender = subMsg.key.participant || subMsg.participant || subSender;
                        const member = metadata.participants.find(p => p.id === realSender || p.id === subSender || p.id.split('@')[0] === subSender.split('@')[0]);
                        const isAdmin = member?.admin === "admin" || member?.admin === "superadmin";

                        if (!isAdmin) {
                            const antilinkMode = config.antilinkMode || {};
                            const mode = antilinkMode[subJid] || "delete";

                            // മെസ്സേജ് ഡിലീറ്റ് ചെയ്യുന്നു
                            try { await subSock.sendMessage(subJid, { delete: subMsg.key }); } catch (delErr) {}

                            if (mode === "warn") {
                                await subSock.sendMessage(subJid, {
                                    text: `⚠️ *@${subSender.split('@')[0]}*, WhatsApp group links are strictly forbidden here!`,
                                    mentions: [subSender]
                                });
                            } 
                            else if (mode === "kick") {
                                await subSock.sendMessage(subJid, { 
                                    text: `🚫 *@${subSender.split('@')[0]} sent a link! Kicking...*`, 
                                    mentions: [subSender] 
                                });
                                ...
                                setTimeout(async () => {
                                    try {
                                        ...
                                        await subSock.groupParticipantsUpdate(subJid, [kickTarget], "remove");
                                    } catch (kickErr) {
                                        console.error("SubBot Kick Error:", kickErr);
                                        await subSock.sendMessage(subJid, { text: `❌ Kick failed! Check bot admin rights or user hierarchy.` });
                                    }
                                }, 1500);
                            }
                            return;
            ...
            if (global.commands) {
                ...
                if (command) {
                    ...
                    if (command.category === "owner" && !isSubOwner) {
                        return await subSock.sendMessage(subJid, { text: "❌ *Owner only!*" }, { quoted: subMsg });
                    }
                    try {
